...
pub async fn get_toolchain_download_url(_client: &Client, target: String) -> String {
    ...
    //     .await
    //     .unwrap();
    // let _tag = json["tag_name"].as_str().expect("Failed to download Succinct toolchain. Likely caused by GitHub rate limiting. Please try again using the --token flag. Docs: https://docs.succinct.xyz/getting-started/install.html#troubleshooting");
